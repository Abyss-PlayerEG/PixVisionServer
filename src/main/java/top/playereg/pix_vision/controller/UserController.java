...
public class UserController {
    ...
    @Operation(
        ...
        description = """
            ...
            ## 参数说明：
            - Authorization: Header 中的 Token，格式为 `Bearer <token>`，或通过 URL 参数 `?token=<token>` 传递
            - dataId: 要删除的数据 ID，Integer 类型，必填

            ## 返回说明：
            - **删除成功**：返回 **{"data": true}** 和“用户拓展数据删除成功”提示
            - **Token 不存在**：返回 **{"data": null}** 和“Token 不存在”提示
            - **Token 已失效**：返回 **{"data": null}** 和“Token 已失效”提示
            - **数据 ID 无效**：返回 **{"data": false}** 和“数据 ID 无效”提示
            - **无权删除**：返回 **{"data": false}** 和“无权删除该数据”提示（数据不属于当前用户）
            ...
            4. 校验数据 ID 参数有效性
            5. 检查用户是否存在
            6. 执行逻辑删除（SQL 层面验证 user_id，确保只能删除自己的数据）
            7. 返回删除结果
            ...
            - 如果数据不属于当前用户，会返回“无权删除该数据”提示
            - 删除后，该数据在查询接口中将不再显示
            """
    )
    public ResponsePojo<Boolean> deleteUserData(
        @Parameter(description = "HTTP 请求对象，用于从 Header 或 URL 参数中获取 Token", required = true) HttpServletRequest request,
        @Parameter(description = "要删除的数据 ID", required = true, example = "1") @RequestParam Integer dataId
    ) {
        // 优先从 URL 参数获取 Token
        String token = request.getParameter("token");
        ...
        String username = JWTUtils.getUsernameFromToken(token);
        log.info("开始删除用户拓展数据，用户 ID: {}, 用户名: {}, 数据 ID: {}", userId, username, dataId);

        // 校验数据 ID 参数
        if (dataId == null || dataId <= 0) {
            log.warn("数据 ID 无效，用户 ID: {}, 数据 ID: {}", userId, dataId);
            return ResponsePojo.error(false, "数据 ID 无效");
        }

        // 调用服务层删除用户拓展数据
        Boolean result = userService.deleteUserData(dataId, userId);

        if (result) {
            log.info("用户拓展数据删除成功，用户 ID: {}, 用户名: {}, 数据 ID: {}", userId, username, dataId);
            return ResponsePojo.success(true, "用户拓展数据删除成功");
        } else {
            // 判断是权限问题还是其他问题
            log.warn("用户拓展数据删除失败，用户 ID: {}, 用户名: {}, 数据 ID: {}", userId, username, dataId);
