...
转载地址: {链接}`
            ...
            9. 生成唯一的文件名（UUID.原始扩展名），根据审核状态添加后缀：.pend（待审核）或 .fail（未过审）
            10. 保存文件到 ~/.pix_vision/data/works/ 目录
            11. 构建 Works 对象并插入数据库
            12. 如果数据库插入失败，删除已上传的文件（事务回滚）
            13. 返回新创建的作品 ID
            ...
            - **新增 AI 文案审核**：发布作品时自动调用 AI 审核服务检测标题和链接，违规作品审核状态为 30（未过审），其他情况为 20（待审核）
            - 审核不通过的作品文件以 .fail 后缀保存，不会公开展示
            """
    )
