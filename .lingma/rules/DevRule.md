...
- **标题格式规范**：一级标题（`#`）必须遵循以下格式：
  - **需要认证的接口**：`功能名称（需要登录认证 + 角色权限[<权限数组>]）`
    - 示例：`批量封禁作品（需要登录认证 + 角色权限[55, 77]）`
    - 示例：`批量删除用户账户（需要登录认证 + 角色权限[77]）`
  - **无需认证的接口**：`功能名称（无需登录认证）`
    - 示例：`分页查询作品列表（无需登录认证）`
    - 示例：`查询单个作品详情（无需登录认证）`
...
MCP 工具的详细使用规范请参考独立文档：
- `.lingma/rules/McpUsage.md` - 核心规则与最佳实践
- `.lingma/skills/mcp-usage/SKILL.md` - 完整使用指南

**重要提醒**：首次使用任何 MCP 工具前必须执行 `Skill: mcp-usage` 查看完整规范。

### 6.1 Context7 MCP 服务使用时机

Context7 是一个远程 MCP 服务，主要用于以下场景：

#### ✅ 适用场景
1. **外部 API 文档查询**：需要查询第三方库、框架的官方文档和 API 参考
2. **技术栈信息获取**：获取最新的技术文档、最佳实践和代码示例
3. **依赖库使用指导**：了解项目中使用的第三方库的正确用法和配置
4. **标准规范查阅**：查询行业标准、协议规范和技术标准文档

#### ❌ 不适用场景
1. **项目内部代码查询**：项目内的代码结构、业务逻辑应通过代码搜索工具查询
2. **数据库操作**：数据库查询应使用 MySQL MCP 服务
3. **缓存管理**：Redis 相关操作应使用 Redis MCP 服务
4. **复杂问题分析**：需要结构化思考的问题应使用 Sequential-Thinking MCP 服务
...
#### ⚠️ 注意事项
- Context7 是远程服务，需要网络连接
- 查询时应提供准确的 libraryId 和具体的查询问题
- 优先查询项目中实际使用的技术栈相关文档
