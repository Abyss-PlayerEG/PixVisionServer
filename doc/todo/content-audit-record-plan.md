...
### 8.1 Swagger 文档更新
- [ ] AdminWorksController.getAdminWorksPage() — description 说明返回 audit_reason/insult_words
- [ ] AdminCommentsController.getCommentsPage() — 同上
- [ ] AdminSeriesController.getAdminSeriesPage() — 同上
- [ ] AdminUserDataChangeController.getPendingPage() — 同上

### 8.2 VO 字段添加 @Schema 注解
- [ ] AdminWorkVO.audit_reason + insult_words
- [ ] AdminCommentVO.audit_reason + insult_words
- [ ] AdminSeriesVO.audit_reason + insult_words
- [ ] UserDataChangeLockVO.audit_reason + insult_words

### 8.3 insult_words JSON 序列化转换
- 入库：`JSON.toJSONString(auditResult.getInsult_words())` → DB varchar
- 出库：保持 JSON 字符串，前端自行解析或直接展示

### 8.4 XML 空列表保护
- `selectLatestByContentIds` 中 `<if test="contentIds != null and contentIds.size() > 0">` 包裹 IN 子句

### 8.5 关闭驼峰映射确认
- 项目使用 snake_case 命名，实体字段 `audit_reason` / `insult_words` 与 DB 列名一致
- ContentAuditRecord 无需 `@TableField` 显式指定列名
- 无需 `@TableLogic`（审核记录只追加不删除）
