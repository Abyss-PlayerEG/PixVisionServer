...
一个现代化的数字艺术平台后端服务，支持用户管理、作品管理、互动评论、系统监控等功能
...
| 持久层 | MyBatis-Plus | 3.5.7 | 增强版 MyBatis |
...
| 系统信息 | http://localhost:9090/system-info | JVM/OS/CPU 信息 |
...
| 表名 | 中文名 | 主要字段 | 状态 |
|------|--------|----------|:----:|
| `tb_user` | 用户账户 | id, username, email, password, user_role, status | 已完成 |
| `tb_user_data` | 用户拓展数据 | id, user_id, data_name, data_content | 已完成 |
| `tb_works` | 作品 | id, title, description, author_id | 开发中 |
| `tb_comments` | 评论 | id, content, user_id, works_id | 开发中 |
| `tb_like` | 点赞 | id, user_id, target_id | 开发中 |
| `tb_star` | 收藏 | id, user_id, works_id | 开发中 |
