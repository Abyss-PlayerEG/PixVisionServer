...
CREATE TABLE `tb_comments` (
  ...
  -- 优化：复合索引提升评论查询性能
  KEY `idx_comments_work_status` (`work_id`, `is_delete`, `approval_status`),
  KEY `idx_comments_parent_floor` (`in_comment_id`, `comment_floor`, `is_delete`),
...
--
-- Table structure for table `tb_content_audit_record`
--

DROP TABLE IF EXISTS `tb_content_audit_record`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `tb_content_audit_record` (
  `record_id` int NOT NULL AUTO_INCREMENT COMMENT '审核记录 ID',
  `content_type` int NOT NULL COMMENT '内容类型：100-作品、200-评论、300-系列、400-昵称',
  `content_id` int NOT NULL COMMENT '对应内容 ID（work_id/comment_id/series_id/lock_id）',
  `original_content` varchar(512) CHARACTER SET utf8mb4 COLLATE utf8mb4_german2_ci NOT NULL COMMENT '原文',
  `approval_status` int NOT NULL COMMENT '审核结果：10-通过、20-待审核、30-违规',
  `audit_reason` varchar(500) CHARACTER SET utf8mb4 COLLATE utf8mb4_german2_ci DEFAULT NULL COMMENT 'AI 审核判断依据',
  `insult_words` varchar(1000) CHARACTER SET utf8mb4 COLLATE utf8mb4_german2_ci DEFAULT NULL COMMENT '命中敏感词，JSON数组字符串',
  `create_time` timestamp NOT NULL COMMENT '审核时间',
  PRIMARY KEY (`record_id`),
  KEY `idx_content` (`content_type`,`content_id`),
  KEY `idx_type_status` (`content_type`,`approval_status`),
  KEY `tb_content_audit_record_tb_comments_FK` (`content_id`)
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_german2_ci COMMENT='AI内容审核记录表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_content_audit_insert` BEFORE INSERT ON `tb_content_audit_record` FOR EACH ROW BEGIN
    DECLARE v_count INT DEFAULT 0;
    CASE NEW.content_type
        WHEN 100 THEN
            SELECT COUNT(*) INTO v_count FROM db_pix_vision.tb_works WHERE work_id = NEW.content_id;
        WHEN 200 THEN
            SELECT COUNT(*) INTO v_count FROM db_pix_vision.tb_comments WHERE comment_id = NEW.content_id;
        WHEN 300 THEN
            SELECT COUNT(*) INTO v_count FROM db_pix_vision.tb_series WHERE series_id = NEW.content_id;
        WHEN 400 THEN
            SELECT COUNT(*) INTO v_count FROM db_pix_vision.tb_user_data_change_lock WHERE lock_id = NEW.content_id;
        ELSE
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Invalid content_type: must be 100/200/300/400';
    END CASE;
    IF v_count = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Foreign key violation: content_id not found in referenced table';
    END IF;
END */;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_content_audit_update` BEFORE UPDATE ON `tb_content_audit_record` FOR EACH ROW BEGIN
    DECLARE v_count INT DEFAULT 0;
    CASE NEW.content_type
        WHEN 100 THEN
            SELECT COUNT(*) INTO v_count FROM db_pix_vision.tb_works WHERE work_id = NEW.content_id;
...
CREATE TABLE `tb_history` (
  ...
  -- 优化：复合索引提升用户历史查询性能
  KEY `idx_history_user_delete_time` (`user_id`, `is_delete`, `time`),
...
CREATE TABLE `tb_like` (
  ...
  -- 优化：复合索引提升用户点赞查询性能
  KEY `idx_like_user_work_delete` (`user_id`, `work_id`, `is_delete`),
...
CREATE TABLE `tb_messages` (
  ...
  `message` text CHARACTER SET utf8mb4 COLLATE utf8mb4_german2_ci COMMENT '消息内容',
  ...
  `message_type` varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_german2_ci NOT NULL DEFAULT 'system' COMMENT '消息类型：system-系统通知、private-私信',
  `ref_id` int DEFAULT NULL COMMENT '关联实体ID（如作品ID、评论ID等）',
  `to` int DEFAULT NULL COMMENT '接收者',
  `is_read` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否已读，0 - 未读、1 - 已读',
  `is_delete_by_sender` tinyint(1) NOT NULL DEFAULT '0' COMMENT '发送者删除标记：0-未删除、1-已删除',
  `is_delete_by_receiver` tinyint(1) NOT NULL DEFAULT '0' COMMENT '接收者删除标记：0-未删除、1-已删除',
  ...
  KEY `idx_msg_receiver_delete` (`to`,`is_delete_by_receiver`),
  -- 优化：复合索引提升消息查询性能
  KEY `idx_msg_receiver_type_read_delete` (`to`, `message_type`, `is_read`, `is_delete_by_receiver`),
  KEY `idx_msg_receiver_sender_type` (`to`, `from_user_id`, `message_type`),
  CONSTRAINT `tb_messages_tb_user_FK` FOREIGN KEY (`to`) REFERENCES `tb_user` (`user_id`)
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_german2_ci COMMENT='消息列表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `tb_series` (
  ...
  -- 优化：复合索引提升用户系列查询性能
  KEY `idx_series_user_status` (`user_id`, `is_delete`, `approval_status`),
  CONSTRAINT `tb_series_tb_user_FK` FOREIGN KEY (`user_id`) REFERENCES `tb_user` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_german2_ci COMMENT='作品系列';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `tb_star` (
  ...
  KEY `tb_history_tb_works_FK` (`work_id`) USING BTREE,
  -- 优化：复合索引提升用户收藏查询性能
  KEY `idx_star_user_work_delete` (`user_id`, `work_id`, `is_delete`),
...
CREATE TABLE `tb_sys_logs` (
  ...
  -- 优化：复合索引提升用户操作日志查询性能
  KEY `idx_syslogs_user_time` (`user_id`, `log_datetime`),
...
CREATE TABLE `tb_user` (
  ...
  `create_user` int NOT NULL DEFAULT '0' COMMENT '存储创建者id，系统创建为0',
  PRIMARY KEY (`user_id`),
  KEY `tb_user_username_IDX` (`username`) USING BTREE,
  KEY `tb_user_user_uuid_IDX` (`user_uuid`) USING BTREE
...
CREATE TABLE `tb_user_data_change_lock` (
  `lock_id` int NOT NULL AUTO_INCREMENT COMMENT 'id唯一值',
  `user_id` int NOT NULL COMMENT '用于链接到对应的用户数据',
  `type` int NOT NULL COMMENT '类型：100-昵称、200-权限、300-头像',
  `nickname` varchar(48) CHARACTER SET utf8mb4 COLLATE utf8mb4_german2_ci DEFAULT NULL COMMENT '待审核昵称，12位长度的中文、英文、特殊字符',
  `user_role` int DEFAULT NULL COMMENT '修改的用户角色：\r\n普通用户 - 11\r\n创作者 - 22\r\n审核员 - 55\r\n售票管理员 - 66\r\n系统管理员 - 77',
  `avatar_url` varchar(96) CHARACTER SET utf8mb4 COLLATE utf8mb4_german2_ci DEFAULT NULL COMMENT '更改的用户头像路径\n存储：/data/userdata/uuid.*\n数据库映射：uuid.*\n限制文件类型：jpg、jpeg、png\n限制大小：2.5mb',
  `old_data` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_german2_ci NOT NULL COMMENT '旧数据',
  `approval_status` int NOT NULL DEFAULT '20' COMMENT '审核状态，10 - 通过、20 - 待审核、30 - 未过审',
  `is_delete` tinyint(1) NOT NULL DEFAULT '0' COMMENT '数据条目删除标签，0 - 未删除、1 - 已删除',
  `create_time` timestamp NOT NULL COMMENT '数据条目创建时间戳',
  PRIMARY KEY (`lock_id`),
  KEY `tb_user_data_change_lock_tb_user_FK` (`user_id`),
  KEY `tb_user_data_change_lock_type_IDX` (`type`) USING BTREE,
  -- 优化：复合索引提升审核查询性能
  KEY `idx_lock_user_type_status` (`user_id`, `type`, `approval_status`),
...
CREATE TABLE `tb_works` (
  ...
  `thumb_url` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_german2_ci NOT NULL COMMENT '封面存储：/data/work_img/uuid.webp\n数据库映射：uuid.webp\n限制文件类型：jpg、jpeg、png\n限制大小：12mb',
  ...
  -- 优化：复合索引提升首页和用户作品查询性能
  KEY `idx_works_status_time` (`is_delete`, `approval_status`, `create_time`),
  KEY `idx_works_user_status` (`user_id`, `is_delete`, `approval_status`),
...
-- Dump completed on 2026-06-13 14:40:47
